...
func TestBuildFailsWhenOutputCannotBeWritten(t *testing.T) {
	...
	if err := os.MkdirAll(utils.JoinFsPath(chunk, "blocker"), 0755); err != nil {
		t.Fatal(err)
	}
	...
	if leftovers, _ := filepath.Glob(utils.JoinFsPath(filepath.Dir(chunk), ".esbuild-*.tmp")); len(leftovers) > 0 {
...
func buildChunkApp(t *testing.T) (*types.ConfigT, string, []byte) {
	...
	for name, content := range files {
		if err := os.WriteFile(utils.JoinFsPath(root, name), []byte(content), 0644); err != nil {
			t.Fatal(err)
		}
	}
	...
	chunks, _ := filepath.Glob(utils.JoinFsPath(root, "public/assets/_asset_chunks/chunk-*.js"))
	if len(chunks) != 1 {
