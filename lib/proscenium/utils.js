import { extname, resolve as resolvePath } from 'std/path/mod.ts'
...
export function loaderType(filename) {
  ...
  return loaderMap[key] || extname(filename).slice(1)
}
...
export async function resolveImport(params, build, importMap) {
  ...
  if (importMap) {
    ...
    if (mappedPath) {
      if (httpRegex.test(mappedPath)) {
        ...
        // The new mapped path should use the CWD (app root) as the resolveDir.
  ...
  // This improves performance, as it avoids a build.resolve() call. But it assumes that the module
  // has no side effects.
  if (ext !== '' && result.path.startsWith('.')) {
  ...
  if (result.path.startsWith('/')) {
    ...
    if (ext !== '' && /\.m?jsx?/.test(ext)) {
      // This improves performance, as it avoids a build.resolve() call. But it assumes that the module
      // has no side effects.
      return { path: result.path, sideEffects: false }
    }
  }
