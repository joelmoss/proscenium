...
import reactSvgPlugin from './esbuild/react_svg_plugin.js'
...
async function main(path, options = {}) {
  ...
  const params = {
    ...
    plugins: [
      importMap && importMapPlugin({ importMap, debug }),
      reactSvgPlugin({ importMap, debug }),
      bundleAllPlugin({ importMap, debug, runtimeDir }),
      bundlePlugin({ importMap, debug, runtimeDir }),
      envPlugin(),
      npmPlugin({ debug }),
      resolvePlugin({
        runtimeDir,
