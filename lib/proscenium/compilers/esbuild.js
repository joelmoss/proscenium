...
async function main(paths = [], options = {}) {
  ...
  // Detect if there is an application component, and set an ENV var if so. We then use this to load
  // it if it exists. See lib/runtime/render_component.jsx.
  const appComponentPath = join(root, 'app', 'components', 'application.jsx')
  Deno.env.set(
    'PROSCENIUM_APPLICATION_COMPONENT',
    (await fileExists(appComponentPath)) ? 'true' : 'false'
  )
