...
import { build, stop, analyzeMetafile } from 'esbuild'
 ...
 *       Debug output.
 *   --metafile
 *       Write the metafile contents to `meta.json`.
 *   --analysis
 *       Analyze metafile and output its analysis.
 */
if (import.meta.main) {
  ...
  const { _: path, ...options } = parseArgs(Deno.args, {
    ...
    boolean: ['write', 'debug', 'analysis', 'metafile'],
...
async function main(path, options = {}) {
  const { root, write, debug, analysis, metafile } = { write: false, ...options }
  ...
  const params = {
    ...
    metafile: metafile || analysis || write,
    keepNames: !isProd,
    write,
  ...
  if (metafile) {
    ...
    await Deno.writeTextFile('meta.json', metadata)
  }

  if (analysis) {
    const content = await analyzeMetafile(result.metafile)
