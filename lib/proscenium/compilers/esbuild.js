...
async function main(path, options = {}) {
  ...
  if (path.startsWith('/') || /^url:https?:\/\//.test(path)) {
    ...
    if (path.endsWith('.map')) {
      entryPoint = join(root, path.slice(0, -4))
      isSourceMap = true
    } else {
  ...
  if (write) {
    ...
    if (isRuntime || isSourceMap) {
      ...
      if (path.endsWith('.css')) {
        text += `/*# sourceMappingURL=${basename(entryPoint)}.map */`
      } else {
        if (!extname(entryPoint)) {
          text += `//# sourceMappingURL=${basename(path)}.map`
        } else {
          text += `//# sourceMappingURL=${basename(entryPoint)}.map`
        }
      }
