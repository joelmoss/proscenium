 ...
 * @param {string} cwd
 * @param {Object} params
 * @returns {{ path: String, external: boolean } | undefined}
 */
export default function (importMap, cwd, params, external) {
  if (!importMap) return
  ...
  const { matched, resolvedImport } = resolve(params.path, importMap, baseURL)

  if (matched) {
    if (resolvedImport instanceof URL) {
      if (resolvedImport.protocol === 'file:') {
        return { path: resolvedImport.pathname }
      } else {
        return {
          path: external ? `/url:${encodeURIComponent(resolvedImport.href)}` : resolvedImport.href,
          external
        }
      }
    } else {
      return { path: resolvedImport }
    }
  }
}
