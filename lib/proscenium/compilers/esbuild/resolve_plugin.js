    ...
    // Catch all resolution.
    {
      ...
      async callback(params) {
        ...
        if (importKinds.includes(params.kind)) {
          ...
          } else if (isOutsideRoot(result.path, cwd)) {
            // Path is outside root. This will most likely be because it is a "link:" dependency.
            if (isBareModule(params.path)) {
              result.path = `/npm:${params.path}`
              result.external = true
            } else {
              // At this point we cannot return an accessible external URL, so we're forced to
              // bundle it.
