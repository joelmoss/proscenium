...
export default setup('resolve', (build, options) => {
  ...
  function readImportMap() {
    const json = readFile(join(cwd, 'config', 'import_map.json'))
    if (json !== null) {
      return parseImportMap(json, baseURL)
    }

    let js = readFile(join(cwd, 'config', 'import_map.js'))
    if (js !== null) {
      js = eval(js)(Deno.env.get('RAILS_ENV'))
      return parseImportMap(JSON.stringify(js), baseURL)
    }
