...
export default setup('css', (build, options) => {
  ...
  return [
    {
      ...
      async callback(params) {
        return await onLoad(params, { ...options, cwd })
      }
    }
  ]
})

export async function onLoad(args, { cwd, customMedia, ...options }) {
  const isInRoot = args.path.startsWith(cwd)

  // Handle CSS module entry points, ensuring that the generated hash is based on the
  // entrypoint. We do this because the hash that is generated by ruby code for use in HTML
  // only knows about the URL (entrypoint) or real path (resolved symlink).
  const hashPath =
    args.pluginData?.entryPoint || (isInRoot ? args.path.slice(cwd.length + 1) : args.path)
  const hash = await digest(hashPath)

  const relativePath = isInRoot ? args.path.slice(cwd.length) : args.path
  ...
  let cmd = [
    ...
    '--nesting',
    options.env === 'production' && '--error-recovery',
    options.env === 'production' && '--minify',
    '--targets',
    '>= 0.25%'
  ].filter(Boolean)
  ...
  if (code === 0) {
    ...
    if (args.pluginData?.importedFromJs) {
      ...
      const mod = [
        ...
        options.env !== 'production' && `e.before(document.createComment('${relativePath}'));`,
