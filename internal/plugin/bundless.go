...
func Bundless(cfg *types.ConfigT) esbuild.Plugin {
		...
		Setup: func(build esbuild.PluginBuild) {
			...
			resolveWithEsbuild := func(args esbuild.OnResolveArgs, onResolveResult *esbuild.OnResolveResult) bool {
				...
				if utils.IsBareModule(onResolveResult.Path) && helpers.IsInsideNodeModules(args.ResolveDir) {
					...
					if err != nil {
						debug.Debug(cfg.Debug, "EvalSymlinks of ResolveDir failed!", err)
						return false
					}
				...
				debug.Debug(cfg.Debug, "resolveWithEsbuild", args, onResolveResult)
			...
			build.OnLoad(esbuild.OnLoadOptions{Namespace: "rubygems", Filter: ".*"},
				func(args esbuild.OnLoadArgs) (esbuild.OnLoadResult, error) {
					...
					if !utils.PathIsCss(realPath) {
						...
						if err != nil {
							...
							return esbuild.OnLoadResult{},
								fmt.Errorf("could not read %s: %w", args.Path, osCause(err))
						}
					...
					debug.Debug(cfg.Debug, "OnLoad(rubygems):end", result)
					...
					if utils.IsBareModule(result.Path) {
						if aliasedPath, exists := utils.HasAlias(result.Path, cfg); exists {
							...
							if utils.IsRubyGem(result.Path) {
								...
								if _, _, err := utils.ResolveRubyGem(result.Path, cfg); err != nil {
									return result, err
								}
